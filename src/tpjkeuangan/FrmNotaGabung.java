 ...
 * @author  oestadho
 */
public class FrmNotaGabung extends javax.swing.JInternalFrame {
    ...
    void setIsKoreksi(boolean b) {
        ...
        if (isKoreksi) btnGenerateNota.setText("Delete Nota");
    }
    ...
    private void udfDeleteNota() {
        try {
            ...
            if(JOptionPane.showConfirmDialog(this, "Anda yakin untuk menghapus nota ini?\n" +
                    "Penghapusan nota ini akan membuat nota yang digabungkan menjadi nota single", "Confirm", JOptionPane.YES_NO_OPTION)==JOptionPane.YES_OPTION){
...
//                lblTglBerangkat.setText("");
    ...
    private void udfPrint() {
        try {
            ...
            String sRpt="nota_cetak_gabungan_v53";
    ...
    private void udfProsesKoreksi(){
        ...
        if(!sOldNota.equalsIgnoreCase(txtNoNota.getText())){
            try{
                ResultSet rs=conn.createStatement().executeQuery("select * from nota where no_nota='"+txtNoNota.getText()+"'");
        ...
        try {
            ...
            stH = conn.createStatement(ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_UPDATABLE);
...
//            rsD=stD.executeQuery("select * from nota_detail limit 0");
            ...
            for (int s=0; s<modelItem.getRowCount(); s++){
                ...
                sInsert+="Insert into nota_detail(no_nota, merk, item_trx, satuan, ket_satuan, ukuran, tarif, sub_total) values " +
                        ...
                        (GeneralFunction.udfGetDouble(modelItem.getValueAt(s, 2))!=0? GeneralFunction.udfGetDouble(modelItem.getValueAt(s, 2)) :null)+", " +
                        (GeneralFunction.udfGetDouble(modelItem.getValueAt(s, 5))!=0? GeneralFunction.udfGetDouble(modelItem.getValueAt(s, 5)) :null)+", " +
                        GeneralFunction.udfGetDouble(modelItem.getValueAt(s, 6))+"); " ;
            }
    ...
    private void udfSimpanItemTrx(String sTag) throws SQLException {
        ResultSet rsH=null;
        int i=conn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE).executeUpdate("Delete from nota_detail where no_nota='"+txtNoNota.getText()+"' ");
        
        rsH=conn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE).executeQuery("select * from nota where no_nota='"+txtNoNota.getText()+"'");
...
//            rsD=stD.executeQuery("select * from nota_detail limit 0");
    ...
    private void initComponents() {
        ...
        jPanel3.add(jScrollPane3, new org.netbeans.lib.awtextra.AbsoluteConstraints(91, 31, 300, 30));
        ...
        jPanel3.add(jLabel9, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 66, 81, 20));

        cmbKondisi.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "Port to Door", "Port to Port", "Door to Door" }));
        jPanel3.add(cmbKondisi, new org.netbeans.lib.awtextra.AbsoluteConstraints(90, 66, 130, -1));

        chkStempel.setText("Stempel & TTD");
        jPanel3.add(chkStempel, new org.netbeans.lib.awtextra.AbsoluteConstraints(250, 65, 140, -1));
        ...
        panelItem.add(jScrollPane4, new org.netbeans.lib.awtextra.AbsoluteConstraints(6, 29, 780, 140));
        ...
        panelItem.add(jButton1, new org.netbeans.lib.awtextra.AbsoluteConstraints(5, 170, 130, -1));
        ...
        setBounds((screenSize.width-845)/2, (screenSize.height-592)/2, 845, 592);
    ...
    private void txtNoNotaDetailKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtNoNotaDetailKeyReleased
        try {
            sQry = "select nota.no_nota, " +
                    "coalesce(nama_kapal||', Berangkat Tgl '||fn_tanggal_ind(nota.tgl_nota),'') as kapal_berangkat, " +
                    "sum(coalesce(sub_total,0)) as sub_total " +
                    "from nota " +
                    "inner join nota_detail d on nota.no_nota=d.no_nota " +
                    "left join nota_kapal_tujuan ktj on seri_kapal=serial_kode " +
                    "left join kapal on kapal.kode_kapal=ktj.kode_kapal " +
                    "where coalesce(nota_header,'')='' and nota.no_nota<>'" + txtNoNota.getText() + "'  " +
                    "and customer='" + txtToko.getText() + "' " +
                    "and tagihan_per='" + (radioToko.isSelected() ? "T" : "P") + "' " +
                    "and (nota.no_nota||coalesce(nama_kapal,'')) ilike '%"+txtNoNotaDetail.getText()+"%' " +
                    "and to_char(ktj.tgl_berangkat, 'yyyy-MM-dd')<='" + fmtYMD.format(fmtDMY.parse(lblTglBerangkat.getText())) + "'  " +
                    "group by nota.no_nota, coalesce(nama_kapal||', Berangkat Tgl '||fn_tanggal_ind(nota.tgl_nota),'') order by nota.no_nota";
        } catch (ParseException ex) {
            Logger.getLogger(FrmNotaGabung.class.getName()).log(Level.SEVERE, null, ex);
        }
        System.out.println(sQry);
        fn.lookup(evt, new Object[]{lblKapalBerangkat, lblJmlNota}, sQry,
                txtNoNotaDetail.getWidth()+lblKapalBerangkat.getWidth()+lblJmlNota.getWidth()+18,
                150);
            ...
            switch(evt.getKeyCode()) {
                ...
                default : {
                    if(!evt.getKeyText(evt.getKeyCode()).equalsIgnoreCase("Up") || !evt.getKeyText(evt.getKeyCode()).equalsIgnoreCase("F2")){
                        ...
                        lst.setBounds(this.txtItemTrx.getLocationOnScreen().x,
                                txtItemTrx.getLocationOnScreen().y + txtItemTrx.getHeight(),
                                txtItemTrx.getWidth()+20,
                                100);
...
//        }
    ...
    private void txtKetSatuanKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtKetSatuanKeyReleased
        ...
        fn.lookup(evt, null, sQry, txtKetSatuan.getWidth()+18, 100);
    ...
    private void txtTokoKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtTokoKeyReleased
        fn.lookup(evt, new Object[]{lblToko, lblKepada}, getQuery(), txtToko.getWidth()+lblToko.getWidth()+18, 120);
    ...
    private void txtKapalKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtKapalKeyReleased
        ...
        fn.lookup(evt, new Object[]{lblKapal, lblTglBerangkat, lblKotaTujuan, lblSerialKapal}, sQry,
                txtKapal.getWidth()+lblToko.getWidth()+jLabel12.getWidth()+lblTglBerangkat.getWidth()+18, 150);
...
private void cmbKotaKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_cmbKotaKeyPressed
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new FrmNotaGabung().setVisible(true);
            }
        });
    }
    ...
    private void udfClear() {
        if(modelItem.getRowCount()>0 && !isSave && !isKoreksi){
...
//    }
    ...
    public class MyKeyListener extends KeyAdapter {
        public void keyPressed(KeyEvent evt) {
		    ...
		    if(evt.getSource().getClass().getName().equalsIgnoreCase("JTable")||fn.isListVisible()){
                        return;
                    }
                    if (!lst.isVisible()){
			Component c = findNextFocus();
...
//        
    ...
    private void udfAddItemTrx(){
        ...
        if(!isEditRow){
            modelItem.addRow(new Object[]{
                txtMerk.getText(),
                txtItemTrx.getText(),
                GeneralFunction.udfGetDouble(txtQty.getText()),
                txtSatuan.getText(),
                txtKetSatuan.getText(),
                GeneralFunction.udfGetDouble(txtTarif.getText()),
                GeneralFunction.udfGetDouble(txtSubTotal.getText())
            });
            ...
            modelItem.setValueAt((GeneralFunction.udfGetDouble(txtQty.getText())==0? "": udfGetFloat(txtQty.getText())) , tblItem.getSelectedRow(), 2);
            ...
            modelItem.setValueAt((GeneralFunction.udfGetDouble(txtTarif.getText())==0? "": udfGetFloat(txtTarif.getText()) ), tblItem.getSelectedRow(), 5);
            modelItem.setValueAt(GeneralFunction.udfGetDouble(txtSubTotal.getText()), tblItem.getSelectedRow(), 6);
            isEditRow=false;
        }
    ...
    private void udfSetTotal(){
        ...
        for (i=0; i<modelItem.getRowCount();i++){
            total = total+ fn.udfGetFloat(modelItem.getValueAt(i, 6));
        }
        for (i=0; i<modelNotaLalu.getRowCount();i++){
            total = total+ fn.udfGetFloat(modelNotaLalu.getValueAt(i, 5));
        }
        for (i=0; i<modelNotaBerikut.getRowCount();i++){
            total = total+ fn.udfGetFloat(modelNotaBerikut.getValueAt(i, 5));
        }
    ...
    private String getQuery(){
        ...
        if(radioPengirim.isSelected()){
            sQry=   "select kode_toko, coalesce(nama,'') as nama, coalesce(nama,'') as kepada " +
                    "from toko where kode_toko||coalesce(nama,'') iLike '%"+txtToko.getText()+"%' order by coalesce(nama,'') ";
    ...
    private String udfGetNewNota(){
        ...
        try{
            ...
            Statement stm=conn.createStatement();
//            ResultSet rs=stm.executeQuery("select fn_nota_get_new_kode_tagihan('"+txtToko.getText()+"', " +
//                    "'"+fmtYMD.format(jDateNota.getDate())+"','"+sTag+"')");
    ...
    private void udfSave(){
        ...
        try {
            conn.setAutoCommit(false);
            int i=conn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE)
                    .executeUpdate("Delete from nota_detail where no_nota='"+txtNoNota.getText()+"' ");
            ...
            try {
                ...
                Logger.getLogger(FrmNotaGabung.class.getName()).log(Level.SEVERE, null, ex1);
            }
