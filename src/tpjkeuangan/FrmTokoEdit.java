 ...
 * @author  oestadho
 */
public class FrmTokoEdit extends javax.swing.JDialog {
    private DlgList lst;
    ...
    /** Creates new form FrmTokoEdit */
    public FrmTokoEdit(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        
        txtKode.addKeyListener(kListener);
        txtNama.addKeyListener(kListener);
        //txtMerk.addKeyListener(new MyKeyListener());
    ...
    private void udfSave(){
        if(udfCekbeforeSave()){
            try {
                rs.close();
                rs=stmt.executeQuery("select * from toko where kode_toko='"+txtKode.getText()+"'");
                rs.next();
                if(isNew){
                    Statement stm=conn.createStatement();
                    ResultSet rsm=stm.executeQuery("select fn_get_kode_toko()");
                ...
                rs.updateString("kode_toko", txtKode.getText());
                
                rs.updateString("nama", txtNama.getText());
                rs.updateString("alamat", txtAlamat.getText());
...
//		    }
    ...
    private void initComponents() {
        ...
        jLabel11.setFont(new java.awt.Font("Book Antiqua", 1, 24));
        jLabel11.setText("Toko/ Pengirim");
        getContentPane().add(jLabel11, new org.netbeans.lib.awtextra.AbsoluteConstraints(358, 12, 230, 30));

        jLabel12.setFont(new java.awt.Font("Book Antiqua", 1, 24));
        jLabel12.setForeground(new java.awt.Color(0, 0, 255));
        jLabel12.setText("Toko/ Pengirim");
        getContentPane().add(jLabel12, new org.netbeans.lib.awtextra.AbsoluteConstraints(358, 13, 230, 30));

        jLabel3.setFont(new java.awt.Font("Lucida Console", 1, 36));
        jLabel3.setForeground(new java.awt.Color(204, 255, 255));
        getContentPane().add(jLabel3, new org.netbeans.lib.awtextra.AbsoluteConstraints(-1, 7, 590, 40));

        jLabel13.setForeground(new java.awt.Color(204, 255, 255));
        jLabel13.setText("Mulai Nota");
        getContentPane().add(jLabel13, new org.netbeans.lib.awtextra.AbsoluteConstraints(260, 250, 102, 20));
        ...
        getContentPane().add(jLabel10, new org.netbeans.lib.awtextra.AbsoluteConstraints(-1, -1, 590, 308));
    ...
    private void formWindowOpened(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowOpened
        lst = new DlgList(JOptionPane.getFrameForComponent(this), false);
        ...
        try {
            ...
            rs=stmt.executeQuery("select kode_toko, coalesce(nama,'') as nama, coalesce(alamat,'') as alamat, " +
                    "coalesce(kota,'') as kota, coalesce(nama_kota,'') as nama_kota, coalesce(telp,'') as telp, " +
                    "coalesce(contact_person,'') as contact_person, coalesce(hp,'') as hp, coalesce(termin_pembayaran,0) as termin_pembayaran," +
                    "coalesce(nota_start,0) as nota_start " +
                    "from toko c left join kota on kode_kota=kota where  kode_toko='"+txtKode.getText()+"' ");
            
            if(rs.next()){
                txtKode.setText(rs.getString("kode_toko"));
...
txtNotaStart.setText(String.valueOf(udfGetInt(txtNotaStart.getText())));
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new FrmTokoEdit(new javax.swing.JFrame(), true).setVisible(true);
            }
        });
    }
...
//        }
        if(txtNama.getText().equalsIgnoreCase("")){
            JOptionPane.showMessageDialog(this, "Silakan isi nama toko terlebih dulu!");
    ...
    void setSrcTable(JTable tblToko) {
        srcTable=tblToko;
    }
